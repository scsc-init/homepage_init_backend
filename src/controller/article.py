...
async def create_article_ctrl(session: SessionDep, body: BodyCreateArticle, user_id: str, user_role: int) -> ArticleResponse:
    ...
    if user_role < board.writing_permission_level:
        raise HTTPException(
            status_code=403, detail="You are not allowed to write this article",)

    article = Article(title=body.title, author_id=user_id,
    ...
    try:
        ...
        logger.error(
            f'err_type=create_article_ctrl ; failed to write file ; {article.id=}', exc_info=True)
    logger.info(
        f'info_type=article_created ; article_id={article.id} ; title={body.title} ; author_id={user_id} ; board_id={body.board_id}')
    article_response = ArticleResponse(
        **article.model_dump(), content=body.content)
