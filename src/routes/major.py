...
async def get_all_majors(session: SessionDep):
    return session.exec(select(Major)).all()

@major_router.get('/major/{id}', tags=['major'])
async def get_major_by_id(id: int, session: SessionDep):
...
@major_router.post('/executive/major/update/{id}', tags=['major'], status_code=200)
async def update_major(id: int, body: BodyCreateMajor, session: SessionDep):
    major = session.get(Major, id)
    if not major:
        raise HTTPException(status_code=404, detail="major not found")
...
async def delete_major(id: int, session: SessionDep):
    major = session.get(Major, id)
    if not major:
        raise HTTPException(status_code=404, detail="major not found")
