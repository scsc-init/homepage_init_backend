...
async def handover_pig(id: int, session: SessionDep, request: Request, body: BodyHandoverPIG) -> None:
    ...
    if not user:
        raise HTTPException(
            status_code=404, detail="새로운 시그/피그장은 해당 시그/피그의 구성원이어야 합니다")
    if current_user.role < get_user_role_level('executive') and current_user.id != pig.owner:
...
async def join_pig(id: int, session: SessionDep, request: Request):
    ...
    if pig.status not in allowed:
        raise HTTPException(400, f"시그/피그 상태가 {allowed}일 때만 시그/피그에 가입할 수 있습니다")
...
async def leave_pig(id: int, session: SessionDep, request: Request):
    ...
    allowed = (ctrl_status_available.join_sigpig_rolling_admission if pig.is_rolling_admission else ctrl_status_available.join_sigpig)
    if pig.status not in allowed:
        raise HTTPException(400, f"시그/피그 상태가 {allowed}일 때만 시그/피그에서 탈퇴할 수 있습니다")
    if pig.owner == current_user.id:
        raise HTTPException(409, detail="시그/피그장은 해당 시그/피그를 탈퇴할 수 없습니다")
    pig_members = session.exec(select(PIGMember).where(
        PIGMember.ig_id == id).where(PIGMember.user_id == current_user.id)).all()
    if not pig_members:
...
async def executive_leave_pig(id: int, session: SessionDep, request: Request, body: BodyExecutiveLeavePIG):
    ...
    if pig.owner == user.id:
        raise HTTPException(409, detail="시그/피그장은 해당 시그/피그를 탈퇴할 수 없습니다")
    pig_members = session.exec(select(PIGMember).where(
        PIGMember.ig_id == id).where(PIGMember.user_id == body.user_id)).all()
    if not pig_members:
        raise HTTPException(404, detail="시그/피그의 구성원이 아닙니다")
    for member in pig_members:
        session.delete(member)
