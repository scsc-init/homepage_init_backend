...
async def handover_pig(
    id: int, request: Request, body: BodyHandoverPIG, pig_service: PigServiceDep
) -> None:
    current_user = get_user(request)
...
async def join_pig(id: int, request: Request, pig_service: PigServiceDep):
    current_user = get_user(request)
    await pig_service.join_pig(id, current_user)


@pig_router.post("/pig/{id}/member/leave", status_code=204)
async def leave_pig(id: int, request: Request, pig_service: PigServiceDep):
    current_user = get_user(request)
...
async def executive_join_pig(
    id: int, request: Request, body: BodyExecutiveJoinPIG, pig_service: PigServiceDep
):
    current_user = get_user(request)
...
async def executive_leave_pig(
    id: int, request: Request, body: BodyExecutiveLeavePIG, pig_service: PigServiceDep
):
    current_user = get_user(request)
