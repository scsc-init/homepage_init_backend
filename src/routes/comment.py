...
async def create_comment(
    session: SessionDep, request: Request, body: BodyCreateComment
) -> Comment:
...
async def get_comments_by_article(
    article_id: int, session: SessionDep, request: Request
...
async def update_comment_by_author(
    id: int, session: SessionDep, request: Request, body: BodyUpdateComment
) -> None:
    comment = session.get(Comment, id)
    if not comment:
        raise HTTPException(
