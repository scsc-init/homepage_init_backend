...
comment_executive_router = APIRouter(
    prefix="/executive/comment", tags=['executive'])
...
async def create_comment(session: SessionDep, request: Request, body: BodyCreateComment) -> Comment:
    ...
    if current_user.role < board.writing_permission_level:
        raise HTTPException(
            status_code=403, detail="You are not allowed to write this comment", )

    comment = Comment(content=body.content, author_id=current_user.id,
                      article_id=article.id, parent_id=body.parent_id)
    session.add(comment)
    try:
        session.commit()
...
async def get_comments_by_article(article_id: int, session: SessionDep, request: Request) -> Sequence[CommentResponse]:
    ...
    if current_user.role < board.reading_permission_level:
        raise HTTPException(
            status_code=403, detail="You are not allowed to read these comments", )
...
async def get_comment_by_id(id: int, session: SessionDep, request: Request) -> CommentResponse:
    ...
    if not comment:
        raise HTTPException(
            status_code=404, detail=f"Comment {id} does not exist")
    ...
    if current_user.role < board.reading_permission_level:
        raise HTTPException(
            status_code=403, detail="You are not allowed to read this comment", )
