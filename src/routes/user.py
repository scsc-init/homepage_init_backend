...
async def update_my_profile(
    session: SessionDep, request: Request, body: BodyUpdateMyProfile
) -> None:
    current_user = get_user(request)
...
async def delete_my_profile(session: SessionDep, request: Request) -> None:
    ...
    if current_user.role >= get_user_role_level("executive"):
        raise HTTPException(
            403,
            detail="임원진 이상의 권한을 가진 사람은 휴회원으로 전환할 수 없습니다.",
        )
...
async def update_user(
    id: str, session: SessionDep, request: Request, body: BodyUpdateUser
) -> None:
    current_user = get_user(request)
    user = session.get(User, id)
    if user is None:
...
async def process_standby_list_manually(
    session: SessionDep, request: Request, body: ProcessStandbyListManuallyBody
) -> None:
    current_user = get_user(request)
...
@user_router.post(
    "/executive/user/standby/process", response_model=ProcessStandbyListResponse
)
...
@user_router.post(
    "/executive/user/standby/process/deposit", response_model=ProcessDepositResponse
)
async def process_deposit(
    session: SessionDep, body: DepositDTO
