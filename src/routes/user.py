    ...
    if discord_id is not None:
        if discord_id == "":
            ...
            try:
                ...
                raise HTTPException(422, detail=[{"type": "int_parsing", "loc": [
                                    "query", "discord_id"], "msg": "Input should be a valid integer, unable to parse string as an integer", "input": discord_id}])
...
async def delete_my_profile(session: SessionDep, request: Request) -> None:
    ...
    if current_user.role >= get_user_role_level('executive'):
        raise HTTPException(403, detail="임원진 이상의 권한을 가진 사람은 휴회원으로 전환할 수 없습니다.")
    current_user.role = get_user_role_level('dormant')
    session.add(current_user)
    session.commit()
    session.refresh(current_user)
    if current_user.discord_id:
        await change_discord_role(session, current_user.discord_id, 'dormant')
    return
...
async def update_user(id: str, session: SessionDep, request: Request, body: BodyUpdateUser) -> None:
    ...
    if (current_user.role <= user.role) and not (current_user.role == user.role == 1000):
        raise HTTPException(
            403, detail=f"Cannot update user with a higher or equal role than yourself, current role: {current_user.role}, {user.email}, user role: {user.role}")
    if body.role:
        level = get_user_role_level(body.role)
        if current_user.role < level:
            raise HTTPException(
                403, detail="Cannot assign role higher than yours")
        user.role = level
    ...
    if body.role:
        ...
        if user.discord_id:
            await change_discord_role(session, user.discord_id, body.role)
    return
...
async def process_standby_list(session: SessionDep, file: UploadFile = File(...)) -> ProcessStandbyListResponse:
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                400, detail="파일 읽기 실패: utf-8, euc-kr 인코딩만 읽을 수 있습니다")
