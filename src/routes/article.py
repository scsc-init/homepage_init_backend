...
async def update_article_by_author(id: int, session: SessionDep, request: Request, body: BodyUpdateArticle) -> None:
    ...
    logger.info(
        f'info_type=article_updated ; article_id={article.id} ; title={body.title} ; '
        f'revisioner_id={current_user.id} ; board_id={body.board_id}'
    )
    try:
...
async def delete_article_by_author(id: int, session: SessionDep, request: Request) -> None:
    ...
    logger.info(
        f'info_type=article_deleted ; article_id={article.id} ; title={article.title} ; '
        f'remover_id={current_user.id} ; board_id={article.board_id}'
    )
    session.commit()
