...
article_executive_router = APIRouter(
    prefix="/executive/article", tags=['executive'])
...
async def create_article(session: SessionDep, request: Request, body: BodyCreateArticle) -> ArticleResponse:
    ...
    try:
        ...
        logger.error(
            f'err_type=create_article ; error occurred during connecting to discord ; {body=}', exc_info=True)
    return ret
...
async def get_article_list_by_board(board_id: int, session: SessionDep, request: Request) -> list[ArticleResponse]:
    ...
    articles = session.exec(select(Article).where(
        Article.board_id == board_id)).all()
    result: list[ArticleResponse] = []
    for article in articles:
        if article.is_deleted:
            result.append(ArticleResponse(
                **article.model_dump(), content=DELETED))
        else:
            try:
                ...
                logger.error(
                    f'err_type=get_article_list_by_board ; error occurred during reading a file ; {board_id=} ; {article.id=}', exc_info=True)
                content = ""
            result.append(ArticleResponse(
                **article.model_dump(), content=content))
...
async def get_article_by_id(id: int, session: SessionDep, request: Request) -> ArticleResponse:
    ...
    try:
        ...
        logger.error(
            f'err_type=get_article_by_id ; error occurred during reading a file ; {article.id=}', exc_info=True)
        content = ""
...
async def update_article_by_author(id: int, session: SessionDep, request: Request, body: BodyUpdateArticle) -> None:
    ...
    try:
        ...
        logger.error(
            f'err_type=update_article_by_author ; failed to write file ; {article.id=}', exc_info=True)
...
async def update_article_by_executive(id: int, session: SessionDep, request: Request, body: BodyUpdateArticle) -> None:
    ...
    try:
        ...
        logger.error(
            f'err_type=update_article_by_executive ; failed to write file ; {article.id=}', exc_info=True)
