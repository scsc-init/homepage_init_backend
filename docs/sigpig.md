...
## Transfer PIG Ownership

### Owner Initiated

* **Method**: `POST`
* **URL**: `/api/pig/:id/handover`
* **Request Body** (JSON):
...
* **Notes**:
  * 새로운 소유자는 해당 PIG의 구성원이어야 하며 기존 소유자와 달라야 함
  * 요청자는 현재 PIG장과 동일해야 함
* **Status Codes**:

  * `204 No Content`: 소유자 변경 성공
  * `400 Bad Request`: 새로운 소유자가 기존 소유자와 동일함
  * `401 Unauthorized`
  * `403 Forbidden`: 요청자가 PIG장이 아님
  * `404 Not Found`: PIG / 사용자 / 구성원 정보가 존재하지 않음
  * `422 Unprocessable Content`

### Executive Initiated

* **Method**: `POST`
* **URL**: `/api/executive/pig/:id/handover`
* **Request Body** (JSON):
